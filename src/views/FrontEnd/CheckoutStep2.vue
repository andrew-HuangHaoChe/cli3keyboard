...
export default {
  ...
  methods: {
    getOrder () {
      const vm = this
      const api = `${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_CUSTOMPATH}/order/${vm.orderId}` // 這裡的orderId就是在created所取回來的id
    ...
    payOrder () {
      const vm = this
      const api = `${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_CUSTOMPATH}/pay/${vm.orderId}` // 這裡的orderId就是在created所取回來的id
      vm.$store.dispatch('updateLoading', true)
